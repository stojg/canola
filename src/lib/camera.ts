...
export function createCamera(
  ...
  function look() {
    ...
    cameraState.yawChange = lerp(
      cameraState.yawChange,
      ptr[0] * ptrSensitivity,
      0.5,
    );
    cameraState.pitchChange = lerp(
      cameraState.pitchChange,
      ptr[1] * ptrSensitivity,
      0.5,
    );
    cameraState.transform.rotateXY(
      -cameraState.yawChange,
      -cameraState.pitchChange,
    );
  }
  ...
  const injectContext = regl({
    context: Object.assign({}, cameraState, {
      projection: (ctx: REGL.DefaultContext) =>
        mat4.perspective(
